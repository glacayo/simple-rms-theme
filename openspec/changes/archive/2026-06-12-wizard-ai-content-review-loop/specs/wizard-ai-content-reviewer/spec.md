...
In production (`WP_DEBUG === false`), each section review MUST log only: pass/fail status
and iteration count. Diagnosis codes, per-field scores, token estimates, and any quality
report data MUST NOT appear in production log entries.
...
- THEN the entry contains `status: pass|fail` and `iterations: N` only
...
- AND log entries contain only bounded pass/fail + iteration count
