...
`AI_Provider_Registry::make_provider()` to support those slugs (including Anthropic
for future enablement / extension compatibility). **Selectable** entries in
`list_providers()` are Ollama Cloud, OpenAI, Google Gemini, and OpenRouter —
Anthropic is implemented but **omitted from the selectable list by default** until
a real API key can be smoke-tested (billing minimum deposit + payment processor
failure for the maintainer). Do not delete `class-anthropic-provider.php`.
`provider_exists()` follows `list_providers()`, so REST/UI cannot configure
Anthropic unless it is re-enabled. Credentials reuse
`AI_Credential_Store::get($slug)` (per-site encrypted options). Harness, reviewer,
builders, REST `/ai/models`, and the wizard UI are already provider-agnostic — no
behavioral changes there. New classes autoload by naming convention
(`wizard-init.php:18` maps `Inc\Wizard\OpenAI_Provider` →
`class-openai-provider.php`), so no manual `require`. Satisfies spec
`wizard-ai-providers` (availability, per-site credentials, gating, single-provider
generation, neutral prompt).
...
| Anthropic selectable availability (v1 enablement) | Keep class + `make_provider( 'anthropic' )`; omit from `list_providers()` until real-key smoke | Ship as selectable / delete class | Maintainer cannot obtain a usable Anthropic key (minimum deposit + payment processor failure). Code stays ready; UI/REST cannot select it while disabled. |
...
| `inc/wizard/class-ai-provider-registry.php` | Modify | 4 `make_provider()` branches (incl. Anthropic for future enablement); selectable `list_providers()` labels for Ollama Cloud, OpenAI, Google Gemini, OpenRouter (Anthropic omitted by default). |
...
Registry `list_providers()` adds selectable `{slug:'openai',label:'OpenAI'}` and peers
for Google Gemini and OpenRouter (plus existing Ollama Cloud). `make_provider()` also
supports `anthropic` while that slug stays off the selectable list by default. Selectable
slugs: `ollama-cloud`, `openai`, `google`, `openrouter`. Implemented-but-disabled:
`anthropic`.
...
- [x] Anthropic is implemented but disabled/pending real-key smoke: keep
  `class-anthropic-provider.php` and `make_provider( 'anthropic' )`; omit Anthropic
  from default `list_providers()` so it is not selectable and `provider_exists(
  'anthropic' )` is false until explicitly re-enabled. Reason: Anthropic requires a
  minimum deposit and the maintainer's payment processor is failing — not a code defect.
