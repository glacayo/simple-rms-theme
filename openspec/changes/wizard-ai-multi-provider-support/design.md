...
On `WP_Error` the JS catch clears the select, focuses manual input, shows the warning (gating). Credentials are persisted only after explicit successful validation. Validation is the live `list_models()` call (OpenAI/OpenRouter `GET /v1/models`, Gemini `GET /models`, Anthropic `GET /v1/models`); a curated/empty fallback list never by itself persists the credential.
...
| `inc/wizard/class-openrouter-provider.php` | Create | OpenAI shape; optional `HTTP-Referer`/`X-Title`; `GET /api/v1/models`. |
