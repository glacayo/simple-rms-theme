...
- [ ] 2.1 Create `inc/wizard/class-openai-provider.php` with Bearer auth, `POST /v1/chat/completions`, `GET /v1/models`, and OpenAI response parsing.
...
- [ ] 2.5a Update `inc/wizard/class-ai-provider-registry.php` for OpenAI: add the `openai` label and `make_provider()` branch only with `OpenAI_Provider` present.
